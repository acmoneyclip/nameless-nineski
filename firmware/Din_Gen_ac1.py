...
# --------- SPEAKER on GP17 ---------
speaker = machine.PWM(machine.Pin(17))
speaker.freq(2000)          # 2 kHz tone
speaker.duty_u16(0)         # start silent

def beep_match():
    # short beep on each *new* match
    speaker.duty_u16(20000)   # some volume (0..65535)
    time.sleep_ms(60)
    speaker.duty_u16(0)
...
def manhattan_dist(p1, p2):
    G1, R1, B1 = p1
    G2, R2, B2 = p2
    return abs(G1 - G2) + abs(B1 - B2) + abs(R1 - R2)
...
print("Sniffer ready (byte mode, BEEP on new match).")
...
print("Running startup sanity capture...")
test_px = capture_pixel_reliable(debug=True)
if test_px is None:
    print("STARTUP TEST: no pixel decoded (check GEN is running + wiring)")
else:
    G, R, B, off = test_px
    print("STARTUP TEST: saw pixel G=%3d R=%3d B=%3d (offset=%d)" %
          (G, R, B, off))
